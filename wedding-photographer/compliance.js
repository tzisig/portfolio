(function() {
  ...
  var BTN_COLOR = { bg:'#7a5c63', fg:'#fff', hover:'#694e54', active:'#5a3e44', light:false };
  var C = BTN_COLOR;
  var sizeStep = 0;
  ...
  function addToolbar() {
    ...
    bar.style.cssText = 'position:fixed;top:50%;right:0;transform:translateY(-50%);z-index:999;display:flex;flex-direction:column;gap:4px;padding:8px;border-radius:10px;background:'+C.bg+';';
    ...
    var sizes = [0, 110, 120, 135, 155, 180];
    ...
    css('.max-w-6xl, main, .container', 'padding-right:60px !important');
  }
  ...
  function addCookie() {
    ...
    b.style.cssText = 'position:fixed;bottom:0;left:0;right:0;z-index:100;padding:20px 24px;background:'+C.bg+';color:#fff;font-size:0.95rem;';
    ...
    text.style.cssText = 'flex:1;min-width:260px;margin:0;color:#fff;';
  ...
  function addFooter() {
    document.querySelectorAll('footer').forEach(function(f){
      ...
      d.innerHTML = '<a href="#" style="color:#ffffff;text-decoration:underline;margin:0 8px;">'+t('מדיניות פרטיות','Privacy Policy')+'</a><a href="#" style="color:#ffffff;text-decoration:underline;margin:0 8px;">'+t('תנאי שימוש','Terms of Service')+'</a><a href="#" style="color:#ffffff;text-decoration:underline;margin:0 8px;">'+t('הצהרת נגישות','Accessibility')+'</a>';
  ...
  requestAnimationFrame(function() {
    ...
    if (!activeBtn) activeBtn = document.querySelector('.btn-primary, .cta-btn-primary, a.btn, button.btn, [class*="cta"], input[type="submit"], button[type="submit"], [class*="btn-primary"]');
    if (!activeBtn) activeBtn = document.querySelector('a[class*="btn"], button, [class*="button"]');
    if (activeBtn) {
      var s = getComputedStyle(activeBtn);
      if (s && s.backgroundColor && s.backgroundColor !== 'rgba(0, 0, 0, 0)' && s.backgroundColor !== 'transparent') {
        ...
        if (m && m.length >= 3) {
          ...
          C = { bg:bg, fg:fg, hover:h, active:a, light:light };
        }
      }
    }
    init();
