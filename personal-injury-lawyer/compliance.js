(function() {
  ...
  var BTN_COLOR = (function() {
    var btn = document.querySelector('a.btn, button.btn, [class*="cta"], input[type="submit"], button[type="submit"], [class*="btn-primary"]');
    if (!btn) btn = document.querySelector('a[class*="btn"], button, [class*="button"]');
    if (btn) {
      var s = getComputedStyle(btn);
      if (s && s.backgroundColor && s.backgroundColor !== 'rgba(0, 0, 0, 0)' && s.backgroundColor !== 'transparent') {
        var bg = s.backgroundColor, fg = s.color;
        var m = bg.match(/(\d+)/g);
        if (m && m.length >= 3) {
          var r = parseInt(m[0]), g = parseInt(m[1]), b = parseInt(m[2]);
          var light = (0.2126*r + 0.7152*g + 0.0722*b) > 128;
          var h = light
          ...
          return { bg:bg, fg:fg, hover:h, active:a, light:light };
        }
      }
    }
    return { bg:'#1e3a5f', fg:'#fff', hover:'#2563eb', active:'#1d4ed8', light:false };
  })();
  ...
  function addSkip() {
    ...
    el.style.cssText = 'position:absolute;top:-200px;left:8px;z-index:10000;background:'+C.bg+';color:#000;padding:12px 24px;border-radius:0 0 8px 8px;font-size:1rem;text-decoration:none;';
    el.addEventListener('focus',function(){this.style.top='8px'});
  ...
  function addToolbar() {
    ...
    bar.style.cssText = 'position:fixed;top:50%;right:8px;transform:translateY(-50%);z-index:999;display:flex;flex-direction:column;gap:4px;padding:8px;border-radius:10px;background:'+C.bg+';';
    ...
    function makeBtn(label, text, fn) {
      ...
      btn.title = label;
      btn.style.cssText = 'width:44px;height:44px;border:none;border-radius:6px;font-size:18px;cursor:pointer;display:flex;align-items:center;justify-content:center;background:'+C.bg+';color:'+C.fg+';transition:background 0.15s;';
  ...
  function addCookie() {
    ...
    b.style.cssText = 'position:fixed;bottom:0;left:8px;right:8px;z-index:100;padding:20px 24px;background:'+C.bg+';color:#000;font-size:0.95rem;color:#000;';
    ...
    ok.style.cssText = 'padding:10px 24px;background:'+C.hover+';color:#000;border:none;border-radius:6px;cursor:pointer;font-weight:600;';
    ...
    x.style.cssText = 'padding:6px 12px;background:transparent;color:#000;border:1px solid rgba(255,255,255,0.3);border-radius:4px;cursor:pointer;font-size:1rem;';
  ...
  function addFooter() {
    document.querySelectorAll('footer').forEach(function(f){
      ...
      d.innerHTML = '<a href="#" style="color:inherit;text-decoration:underline;opacity:0.8;margin:0 8px;">'+t('מדיניות פרטיות','Privacy Policy')+'</a><a href="#" style="color:inherit;text-decoration:underline;opacity:0.8;margin:0 8px;">'+t('תנאי שימוש','Terms of Service')+'</a><a href="#" style="color:inherit;text-decoration:underline;opacity:0.8;margin:0 8px;">'+t('הצהרת נגישות','Accessibility')+'</a>';
  ...
  function init() {
    // Dynamic CSS for contrast & stop animations
